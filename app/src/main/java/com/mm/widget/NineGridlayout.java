...
public class NineGridlayout extends ViewGroup {
    ...
    private void layoutChildrenView() {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            childrenView.setImageUrl(listData.get(i).getPath());
    ...
    public void setImagesData(List<ImageEntity> lists) {
    ...
    private CustomImageView generateImageView(final int position) {
        final CustomImageView iv = new CustomImageView(getContext());
        iv.setScaleType(ImageView.ScaleType.CENTER_CROP);
        iv.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                String imageUrl[] = new String[listData.size()];
                for (int i = 0; i < listData.size(); i++) {
                    imageUrl[i] =  RetrofitApiFactory.BASE_URL + listData.get(i).getPath();
                }

                Bundle b = new Bundle();
                b.putStringArray("imageUrls", imageUrl);
                b.putString("curImageUrl", imageUrl[position]);
                int size = FileUtils.findSize(imageUrl, imageUrl[position]);
                if (size == -1) {
                    return;
                }
